...
kRc5T1 = 889
kRc5MinCommandLength = 113778
kRc5MinGap = kRc5MinCommandLength - kRC5RawBits * (2 * kRc5T1)
kRc5ToggleMask = 0x800  # The 12th bit.
kRc5SamplesMin = 11
...
kRc6Tick = 444
kRc6HdrMarkTicks = 6
kRc6HdrMark = kRc6HdrMarkTicks * kRc6Tick
kRc6HdrSpaceTicks = 2
kRc6HdrSpace = kRc6HdrSpaceTicks * kRc6Tick
kRc6RptLengthTicks = 187
kRc6RptLength = kRc6RptLengthTicks * kRc6Tick
kRc6ToggleMask = 0x10000UL  # The 17th bit.
kRc6_36ToggleMask = 0x8000  # The 16th bit.
...
void IRsend.sendRC5(data, nbits, repeat) {
  ...
  for (i = 0 i <= repeat i++) {
    ...
    # Data
    for (mask = 1ULL << (nbits - 1) mask mask >>= 1)
      if (data & mask) {  # 1
...
#   http:#www.sbprojects.com/knowledge/ir/rc5.php
#   https:#en.wikipedia.org/wiki/RC-5
IRsend.encodeRC5(address, command,
                           bool key_released) {
...
#   http:#www.sbprojects.com/knowledge/ir/rc5.php
#   https:#en.wikipedia.org/wiki/RC-5
IRsend.encodeRC5X(address, command,
                            bool key_released) {
...
#   http:#www.sbprojects.com/knowledge/ir/rc5.php
#   https:#en.wikipedia.org/wiki/RC-5
IRsend.toggleRC5(data) { return data ^ kRc5ToggleMask }
#endif  # SEND_RC5
...
#   http:#www.righto.com/2010/12/64-bit-rc6-codes-arduino-and-xbox.html
#   http:#www.pcbheaven.com/userpages/The_Philips_RC6_Protocol/
IRsend.encodeRC6(address, command, mode) {
  switch (mode) {
    case kRC6Mode0Bits:
...
#   http:#www.righto.com/2010/12/64-bit-rc6-codes-arduino-and-xbox.html
#   https:#en.wikipedia.org/wiki/Manchester_code
void IRsend.sendRC6(data, nbits, repeat) {
  ...
  for (r = 0 r <= repeat r++) {
    ...
    # Data
    bitTime
    for (i = 1, mask = 1ULL << (nbits - 1) mask i++, mask >>= 1) {
...
#if (DECODE_RC5 or DECODE_RC6 or DECODE_LASERTAG)
...
# Ref:
#   https:#en.wikipedia.org/wiki/Manchester_code
int16_t IRrecv.getRClevel(decode_results *results, *offset,
                           *used, bitTime, tolerance,
                           int16_t excess, delta, maxwidth) {
  ...
  val = ((*offset) % 2) ? kMark : kSpace
  # Check to see if we have hit an inter-message gap (> 20ms).
  if (val == kSpace and
      (width > 20000 - delta or width > maxwidth * bitTime + delta)) {
...
#endif  # (DECODE_RC5 or DECODE_RC6 or DECODE_LASERTAG)
...
bool IRrecv.decodeRC5(decode_results *results, nbits, bool strict) {
  if (results->rawlen < kRc5SamplesMin + kHeader - 1) return False

  # Compliance
  if (strict and nbits != kRC5Bits and nbits != kRC5XBits)
  ...
  if (levelA == kSpace and levelB == kMark) {  # Matched a 1.
    is_rc5x = False
  } else if (levelA == kMark and levelB == kSpace) {  # Matched a 0.
  ...
  for ( offset < results->rawlen actual_bits++) {
    ...
    if (levelA == kSpace and levelB == kMark)
      data = (data << 1) | 1  # 1
    else if (levelA == kMark and levelB == kSpace)
      data <<= 1  # 0
    else
      break
  }
  ...
  if (strict and actual_bits != kRC5Bits and actual_bits != kRC5XBits)
    return False
...
bool IRrecv.decodeRC6(decode_results *results, nbits, bool strict) {
  ...
  tick = results->rawbuf[offset++] * kRawTick / kRc6HdrMarkTicks
  ...
  for (actual_bits = 0 offset < results->rawlen actual_bits++) {
    ...
    if (actual_bits == 3 and levelA != getRClevel(results, &offset, &used, tick))
      return False
    levelB = getRClevel(results, &offset, &used, tick)
    # T bit is double wide make sure second half matches
    if (actual_bits == 3 and levelB != getRClevel(results, &offset, &used, tick))
      return False
    if (levelA == kMark and levelB == kSpace)  # reversed compared to RC5
      data = (data << 1) | 1                 # 1
    else if (levelA == kSpace and levelB == kMark)
      data <<= 1  # 0
    else
      break
  }
  ...
  # More compliance
  if (strict and actual_bits != nbits)
